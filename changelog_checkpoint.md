...
- P4.9 is decomposed into six roadmap micro-slices, P4.9a through P4.9f, plus the owner-approved P4.9d_expand and P4.9e_expand extensions so choice integration, transient recognition preview, controlled vocabulary, candidate transfer, vocabulary maintenance, and confirmed semantic writes retain explicit acceptance boundaries.
...
- P4.9d explicit Size/Color candidate transfer and its owner-approved P4.9d_expand full vocabulary manager are released, remote-aligned, CI-passed, and `PASSED`. Automatic or silent form filling and automatic alias learning remain deferred.
- P4.9e Product Type and Tag Confirmation Attachment Baseline plus P4.9e_expand Product Type and Tag Vocabulary Management Surface are locally implemented, integrity-audited, automated-verification complete, and owner-reviewed; Prompt 5 release remains pending.
...
- Status: IN_PROGRESS
- Current functional state: P4.9d plus P4.9d_expand are release-closed; P4.9e Product Type and Tag Confirmation Attachment Baseline plus P4.9e_expand Product Type and Tag Vocabulary Management Surface are `AUDITED_READY` after local implementation, combined integrity verification, resolved local migration recovery, and required owner browser acceptance; Prompt 5 release remains pending
- Next functional micro-slice after this combined delivery closes: P4.9f Material Confirmation Attachment Baseline — `NOT_STARTED` and still requires the documented owner decision on exact confirmation UI and alias behavior
- Started: 2026-07-27
...
- P4.1 Semantic Recognition Service Contract Baseline implementation exists and is released.
...
- P4.2 Product Type Recognition Baseline implementation exists and is released.
...
- Result: P4.2 Product Type Recognition Baseline is implemented, integrity-audited, locally verified, committed, pushed, CI-passed, and `PASSED`; exact delivery metadata is read from Git/GitHub.
...
- Files modified by P4.3:
  ...
  - `changelog_checkpoint.md`
- Scope audit result:
  - `catalog.BusinessTag` is a business-owned Tag vocabulary source.
  - Tag names are stripped on save, must contain non-whitespace text, and are unique per Business after case-insensitive trim normalization.
  - `BusinessTag.business` is required and uses `PROTECT`, preserving the Business ownership boundary.
  - Tag recognition reads only the supplied Business vocabulary and returns transient unconfirmed `TAG` candidates through the P4.1 recognition contract.
  - Recognition without a Business returns no Tag candidates.
  - Browser UI is intentionally unchanged because P4.3 is backend-only vocabulary and recognition behavior.
  - No Product tag join, Product form assignment, tag-management UI, aliases, tag readiness policy, material fact persistence, size/color choices, stock, computed availability, buyer replies, public catalog, chatbot, orders, payments, delivery, broad ERP, or AI-truth behavior was added.
- Verification commands run during implementation and integrity audit:
  - `.venv/bin/python manage.py check` passed with `System check identified no issues (0 silenced).`
  - `.venv/bin/python manage.py check --settings=config.settings.test` passed with `System check identified no issues (0 silenced).`
  - `.venv/bin/python manage.py makemigrations --check --dry-run` passed with `No changes detected`.
...
- P4.4 Business-Scoped Alias Normalization Baseline implementation exists and is released.
...
- Verification commands run during implementation and integrity audit:
  ...
  - `git diff --check` passed with no whitespace errors.
- Result: P4.4 Business-Scoped Alias Normalization Baseline is implemented, integrity-audited, locally verified, committed, pushed, CI-passed, and `PASSED`; exact delivery metadata is read from Git/GitHub.

### P4.5a verification

- P4.5a Confirmed Material Fact Model Baseline implementation exists and is released.
- Files created by P4.5a:
  - `catalog/migrations/0005_product_material_fact.py`
- Files modified by P4.5a:
  - `catalog/models.py`
  - `catalog/tests.py`
  - `DEVELOPMENT_NOTES.md`
  - `changelog_checkpoint.md`
- Repairs made during integrity audit:
  - added missing Business deletion protection coverage for material facts.
- Scope audit result:
  - `catalog.ProductMaterialFact` is a confirmed material fact model scoped to both `Business` and `Product`.
  - Canonical material and original seller wording are required and stripped before save.
  - Percentage is optional and limited to `1..100` when present.
  - Source is explicit and limited to the approved backend baseline sources: description or manual.
  - Confirmation state is explicit and confirmed-only for this baseline.
  - Material facts validate that their Product belongs to the same Business.
  - Recognition material candidates remain transient and do not create persisted confirmed facts.
  - Browser UI is intentionally unchanged because P4.5a is backend-only confirmed material fact persistence.
  - No material confirmation UI, material aliases, Product form integration, readiness, buyer replies, search, size/color choices, stock, availability, measurements, public catalog, chatbot, orders, payments, delivery, broad ERP, or AI-truth behavior was added.
- Verification commands run during implementation and integrity audit:
  - `.venv/bin/python manage.py check` passed with `System check identified no issues (0 silenced).`
  - `.venv/bin/python manage.py check --settings=config.settings.test` passed with `System check identified no issues (0 silenced).`
  - `.venv/bin/python manage.py makemigrations --check --dry-run` passed with `No changes detected`.
  - `.venv/bin/python manage.py makemigrations --check --dry-run --settings=config.settings.test` passed with `No changes detected`.
  - `.venv/bin/python manage.py test catalog --settings=config.settings.test -v 2 --noinput` ran 100 tests, passed, and created/destroyed `test_facebook_erp_dev`.
  - `.venv/bin/python manage.py test --settings=config.settings.test -v 2 --noinput` ran 138 tests, passed, and created/destroyed `test_facebook_erp_dev`.
  - `git diff --check` passed with no whitespace errors.
- Result: P4.5a Confirmed Material Fact Model Baseline is implemented, integrity-audited, locally verified, committed, pushed, CI-passed, and `PASSED`; exact delivery metadata is read from Git/GitHub.
...
- Repairs made during implementation:
  - narrowed a new material-term deduplication test to assert the approved case-insensitive invariant instead of database collation-dependent display casing.
- Scope audit result:
  - `catalog.recognition.material_terms_for_business()` derives material recognition terms from confirmed `ProductMaterialFact` canonical materials for the supplied Business only.
  - `catalog.recognition.recognize_materials_for_business()` returns transient unconfirmed `MATERIAL` candidates through the existing recognition contract.
  - Recognition preserves observed text separately from candidate meaning and returns no confirmed facts.
  - Material recognition is read-only and does not create or update `ProductMaterialFact`.
  - Cross-Business material facts do not leak into recognition.
...
- P4.6 Size/Color-to-Choice Suggestion Baseline implementation exists and is released.
...
- Integrity-audit verification commands:
  ...
  - `git diff --check` passed with no whitespace errors.
- Release verification from the Phase 4 documentation reality audit:
  - `HEAD`, local `origin/main`, and remote `main` are aligned at `0bf4c37efe9e98ec6279935a69a952c9f5d0091d`.
...
### P4.7 released baseline and duplicate-policy correction

- Released P4.7 created `catalog.ProductChoice` and migration `0006_product_choice.py`, then passed local and remote CI verification.
- A post-release audit found one real policy defect: the released model, migration, tests, and live documents blocked case-insensitive, trim-normalized duplicate size/color rows for one Product, while the corrected owner policy allows them as distinct sellable rows.
- Corrective files:
  - `catalog/models.py`
  ...
  - `changelog_checkpoint.md`
- Corrected behavior:
  - migration `0006_product_choice.py` and the released commit remain unchanged;
  - forward migration `0007` removes `unique_product_choice_per_product`;
  - duplicate same-Product size/color rows, including case-insensitive trim-normalized matches, persist with distinct primary keys, quantities, and active states;
  - rows are not merged automatically, and future inventory mutations must target a specific `ProductChoice` row;
  - nonblank size/color, nonnegative quantity, Product/Business matching, and active/inactive state remain enforced;
  - aggregation, buyer-facing wording, UI disambiguation, Product form integration, inventory, availability, readiness, replies, measurements, and broader commerce behavior remain deferred.
- Local corrective verification:
  - `.venv/bin/python manage.py check` passed with no issues;
  - migration dry-run checks before and after migrate returned `No changes detected`;
...
### P4.8 implementation and integrity audit
...
- Implemented behavior:
  - restricted Product choice fields and inline formset validation;
  - empty extra rows are ignored while partial rows retain field errors;
  - active Products require at least one retained active choice, while drafts may have none;
...
- Local verification result:
  ...
  - local and test-settings Django system checks passed with no issues;
  - local and test-settings migration dry-run checks passed with `No changes detected`;
  - 17 focused P4.8 tests passed;
  - 140 catalog tests passed;
...
### P4.9 execution map, released P4.9d delivery, and current P4.9e/P4.9e_expand delivery

- P4.9 contains six roadmap micro-slices plus two owner-approved extensions:
...
- P4.9a through P4.9d, including P4.9d_expand, are release-closed; Git/GitHub is authoritative for their exact commits and CI evidence.
- P4.9b implementation areas:
  - `catalog/forms.py`;
  - `catalog/recognition.py`;
  - `catalog/tests.py`;
  - `catalog/views.py`;
...
- Implemented behavior:
  - description changes trigger a debounced automatic HTMX preview with full-page fallback;
  - observed wording, canonical candidate meaning, semantic destination, and confirmation-required state remain distinct;
  - Product Type/Tag vocabulary and aliases, confirmed material facts, and current confirmed ProductChoice size/color values are read only from the active Business;
...
- Automated audit verification:
  ...
  - 32 focused recognition/create/update tests passed in 23.493s;
  - 157 catalog tests passed in 100.185s;
  - 195 full-suite tests passed in 117.130s;
  - `git diff --check` passed.
- Owner/browser local acceptance: `PASS` — automatic recognition displayed the transient `Choice size M` candidate without saving or confirming it.
- Owner-approved roadmap refinement: unrestricted open-text size/color is not the target V1 interaction. P4.9c must introduce Business-scoped seller-managed canonical Size/Color values, multilingual aliases, dropdown selection, contextual value creation, and safe existing-choice migration before P4.9d candidate transfer.
- Result: P4.9b automated integrity and owner/browser acceptance `PASS`; it was subsequently committed, pushed, remote-aligned, and CI-passed.
- P4.9c implementation areas:
  - `catalog/models.py` and migrations `0008`/`0009` add Business-owned canonical Size/Color values and explicit aliases, then replace legacy choice text with protected canonical foreign keys;
  - `catalog/vocabulary.py` owns atomic contextual creation/reactivation and explicit alias collision handling;
  - `catalog/forms.py`, `catalog/product_bundles.py`, and `catalog/views.py` enforce active-Business dropdown querysets, server-controlled ownership, compact contextual creation, and preservation of unsaved Product/choice input;
  - `catalog/recognition.py` reads active canonical values and approved aliases from the active Business while preserving transient candidate semantics;
  - `templates/catalog/product_form.html`, `templates/catalog/_choice_section.html`, and `static/css/app.css` provide the responsive dropdown and contextual vocabulary interaction;
  - `catalog/tests.py` and `catalog/test_migrations.py` cover vocabulary integrity, ownership, create/edit no-write behavior, recognition aliases, dropdown tampering, and forward/reverse migration shape.
- P4.9c migration policy preserves ProductChoice primary keys, Product/Business ownership, quantities, active state, and duplicate row identities. Case/trim-equivalent historical labels within one Business reuse the deterministic first canonical value; semantically different labels are not guessed or merged.
...
- Owner/browser result: P4.9d candidate transfer `PASS`; P4.9d_expand vocabulary management `PASS` on desktop and approximately 390px mobile width.
- Result: P4.9d plus P4.9d_expand were subsequently committed, pushed, remote-aligned, CI-passed, and release-closed; exact delivery metadata remains sourced from Git/GitHub.
- P4.9e plus P4.9e_expand implementation areas:
  - `catalog/models.py` and migrations `0010_product_classification.py`/`0011_product_type_tag_activation.py` add the protected Product classification associations and non-destructive active state for Type/Tag vocabulary;
  - `catalog/forms.py` limits new confirmation to active canonical values from the current Business while retaining existing inactive confirmed values for correction/removal;
  - `catalog/product_bundles.py` persists Product, choice, type, and tag correction/removal inside the existing atomic bundle boundary;
  - `catalog/vocabulary.py`, `catalog/views.py`, and `catalog/recognition.py` provide atomic Business-scoped Type/Tag vocabulary maintenance and exclude inactive vocabulary from new recognition;
  - `templates/catalog/product_form.html`, `templates/catalog/choice_vocabulary.html`, `templates/catalog/product_list.html`, and `static/css/app.css` provide compact confirmed-classification controls, a complete Product vocabulary manager, safe return paths, and responsive presentation;
  - `catalog/tests.py` covers association and vocabulary integrity, cross-Business/tampered values, candidate no-write behavior, validation recovery, deterministic replacement/removal, activation semantics, transaction rollback, and existing-reference preservation.
- Implemented behavior: one optional canonical Product Type and zero or more canonical Tags become confirmed Product truth only after explicit seller selection and a valid Product bundle save. Sellers can create, rename, replace aliases for, and activate/deactivate current-Business Product Types and Tags; deactivation removes values from new confirmation and recognition without deleting existing Product truth. Candidates alone never attach, and canonical deletion/merge, automatic vocabulary or alias learning, material confirmation, inventory, availability, readiness, search, and buyer replies remain excluded.
- Combined hardening: owner review exposed that local PostgreSQL had not applied additive migration `0011`, so Product Add failed on missing activation columns even though fresh test databases passed. The failure was reproduced, the additive local migration was applied without data loss, local migration-state verification passed, and a direct local Product Create render returned `200` with classification and choice controls.
- Combined verification: local/test Django checks and migration dry-runs passed; local migration-state check passed; 165 focused P4.9e/P4.9e_expand tests, 222 catalog tests, and 260 full-suite tests passed; `git diff --check` passed.
- Integrity audit result: `PASS` — Business isolation, atomic Product/vocabulary writes, candidate-versus-confirmed truth, inactive-reference preservation, validation recovery, and hosted Django/PostgreSQL compatibility remain intact with no later-phase behavior.
- Required owner/browser review: `PASS` after local schema recovery. Prompt 5 release remains the only delivery gate.
...
- No P4.9d or P4.9d_expand implementation, acceptance, or release blocker remains; the combined delivery is closed.
- No known P4.9e or P4.9e_expand implementation, verification, integrity, migration-state, or owner-acceptance blocker remains. Prompt 5 release remains pending.
...
- Gate 3 is not passed; the P4.9e/P4.9e_expand release, P4.9f, inventory, and availability work remain incomplete.
...
P4.9 contains six roadmap micro-slices plus the owner-approved P4.9d_expand and P4.9e_expand extensions. P4.9a through P4.9d, including P4.9d_expand, are released and `PASSED`. P4.9e plus P4.9e_expand are `AUDITED_READY` with Prompt 5 release pending; P4.9f remains `NOT_STARTED`.

The current combined delivery is P4.9e Product Type and Tag Confirmation Attachment Baseline plus P4.9e_expand Product Type and Tag Vocabulary Management Surface. Local implementation, automated verification, combined integrity audit, local migration recovery, and required owner browser confirmation passed; Prompt 5 release is the immediate gate. After this delivery is release-closed, the next functional micro-slice is P4.9f Material Confirmation Attachment Baseline, subject to the existing owner decision on its exact confirmation UI and alias behavior.
...
- Operation: combined P4.9e/P4.9e_expand integrity audit, local schema recovery, owner acceptance, and mandatory documentation synchronization.
- Source/test/UI/migration files changed or directly verified:
  - `catalog/models.py`
  - `catalog/migrations/0010_product_classification.py`
  - `catalog/migrations/0011_product_type_tag_activation.py`
  - `catalog/forms.py`
  - `catalog/product_bundles.py`
  - `catalog/recognition.py`
  ...
  - `templates/catalog/product_list.html`
  - `static/css/app.css`
- Documentation updated by the audit: `changelog_checkpoint.md`, `DEVELOPMENT_NOTES.md`, `BUILD_PLAN.md`, `APP_EXPERIENCE_PLAN.md`, and `README.md`.
- Files intentionally not modified: URLs, settings, CI, dependencies, frozen docs, Git history, remote configuration, and unrelated owner-controlled local documents.
- Source prototype modified: no.
- Scope result: active-Business Product Type/Tag vocabulary can be maintained explicitly and selected canonical values can be confirmed, corrected, or removed through the atomic Product bundle without automatic candidate attachment or learning, canonical deletion/merge, material writes, inventory, availability, readiness, search, buyer replies, public, or broader-commerce behavior.
- Verification result: local/test Django checks and migration dry-runs passed; local migration-state check passed after applying missing additive migration `0011`; direct local Product Create render returned `200`; 165 focused tests, 222 catalog tests, and 260 full-suite tests passed; `git diff --check` passed; required owner/browser acceptance passed.
...
- Last released functional milestone: P4.9d Size/Color Candidate-to-Choice Transfer Baseline plus P4.9d_expand Size/Color Vocabulary Management Surface are committed, pushed, remote-aligned, CI-passed, and `PASSED`; exact current delivery metadata is read from Git/GitHub.
- Pending release milestone: P4.9e Product Type and Tag Confirmation Attachment Baseline plus P4.9e_expand Product Type and Tag Vocabulary Management Surface — `AUDITED_READY`; Prompt 5 release remains pending.
- Next functional milestone after pending delivery closure: P4.9f Material Confirmation Attachment Baseline — `NOT_STARTED` and subject to its existing owner decision.
...
7. confirm Phase 3 is `PASSED`, Phase 4 is `IN_PROGRESS`, and P3.1 through P3.4 plus P4.1 through P4.9d, including P4.9d_expand, are released and `PASSED`;
8. confirm Gate 3 is not passed because the P4.9e/P4.9e_expand release, P4.9f, inventory, and availability remain incomplete;
9. confirm P4.9e plus P4.9e_expand are `AUDITED_READY` after required owner browser review passed, with Prompt 5 release still pending; P4.9f remains next only after this delivery closes and its existing owner decision is resolved;
