...
def _validate_choice_product_is_mutable(choice):
    if choice.product.lifecycle == Product.Lifecycle.ARCHIVED:
        raise ValidationError("Archived Product stock cannot be changed.")
...
def apply_choice_quantity_delta(*, business, choice, actor, delta):
    ...
    with transaction.atomic():
        try:
            locked_choice = (
                ProductChoice.objects.select_for_update(of=("self", "product"))
        ...
        _validate_choice_product_is_mutable(locked_choice)
