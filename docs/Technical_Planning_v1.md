...
| `catalog` | Product, product choices, photos, taxonomy, lifecycle, product workspace queries | Keep views thin |
| `clothing` | Clothing-specific product attributes, category measurement templates, choice attributes, and fit guidance | Keep domain profile boundary explicit; follow `docs/domain/CLOTHING_DATA_SPEC_V1.md` |
...
- RECOMMENDED_FROM_EVIDENCE: `Product` as catalog item with lifecycle, name/description, base price, currency, and timestamps.
- RECOMMENDED_FROM_EVIDENCE: `ProductChoice` or `ProductVariant` as the stock-bearing choice record.
- RECOMMENDED_FROM_EVIDENCE: `ProductPhoto` as optional image/gallery record with controlled primary-photo invariant.
- RECOMMENDED_FROM_EVIDENCE: Clothing domain data should follow `docs/domain/CLOTHING_DATA_SPEC_V1.md`.
- RECOMMENDED_FROM_EVIDENCE: `ClothingProductProfile` and `ClothingChoiceProfile` for clothing-specific target audience, type, product-level garment attributes, category measurements, fit guidance, size, and color.
...
| Clothing product attributes | Clothing product profile or approved related profile | Readiness/reply services when present | Forms, cards, replies if approved | Seller-maintained facts only |
| Garment measurements | Category-specific clothing measurement structure | Readiness/reply services when present | Forms and deterministic replies if approved | No global requirement for every product |
| Fit guidance | Seller-entered fit guidance fields | Reply service when present | Forms and deterministic replies if approved | Weight alone must not determine size |
...
| Photo | Product photo record | Primary-photo selector | Cards, forms, readiness | One primary photo policy |
| Readiness | None | Readiness service | Dashboard, cards, replies | Computed from stored facts only |
| Buyer reply | None | Deterministic reply service | Seller copy UI | No invented facts |
...
- Clothing data: explicit service/schema boundary for product-level attributes, category measurement templates, variant data, and fit guidance.
...
- Deterministic answer: reply service that consumes stored facts and computed availability/readiness.
...
- RECOMMENDED_FROM_EVIDENCE: Clothing measurements are category-specific; do not require every measurement for every category.
...
- OWNER_DECISION_REQUIRED: Required/optional/deferred clothing attributes and measurement templates.

## 13. Testing Strategy

Required layers:

- Unit tests: pure helpers, normalization, display labels.
- Service tests: availability, readiness, deterministic replies, inventory mutation, clone.
...
- Clothing-domain tests: required/optional category measurements, no all-fields-required behavior, variant stock separation, fit guidance wording.
...
| Readiness missing data | No false ready state | Service/view | Yes |
...
- Which clothing attributes and category measurement templates are V1.
