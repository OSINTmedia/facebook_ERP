...
- OWNER_APPROVED_BOUNDARY: In the current Product create/edit flow, saved stock quantity is read-only; unsaved choices expose non-negative Starting stock, existing quantities are managed separately, persisted choices are deactivated rather than deleted, and unsaved extra rows can be discarded.
- OWNER_APPROVED_P5.6: Product edit may place compact `+1`/`-1` controls beside persisted choices only. The quantity input remains read-only; every action uses the separate authenticated inventory mutation route, replaces only its own control region from server truth, exposes loading/status or error feedback, and has a safe non-HTMX fallback. Unsaved rows have no stock control.
- OWNER_OBSERVED_P5.6_UX_GAP_RESOLVED_BY_P5.6A: Owner/browser testing confirmed that the one-save Starting stock correction removes the mandatory save-at-zero step while retaining persisted choice identity, server-owned quantity, and the immutable adjustment trail. The broader assistant-style Product UX remains later work.
- OWNER_APPROVED_P5.6A: An unsaved choice on Product create or edit exposes a clearly labeled `Starting stock` non-negative integer field in the normal Product form. The seller completes Product facts, choices, and each new choice's starting stock with one final save; invalid submissions and no-write helper interactions preserve the entered value. On save, the server first persists the choice at zero and records any positive start as one atomic `0 -> N` inventory fact owned by the active Business and actor. A saved choice never exposes direct quantity editing and continues to use the P5.6 `-1`/`+1` controls. This is one-time initialization, not a general direct-set capability.
