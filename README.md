...
After local PostgreSQL credentials are configured in `.env`, run `source .venv/bin/activate && python manage.py runserver 127.0.0.1:8000` to review the auth-gated shell locally. Anonymous users are redirected to the login page; after signing in, the root route renders the base application shell, the Products nav opens the Product list, and Product create/edit supports controlled Business-scoped Size/Color dropdowns, contextual approved values and aliases, read-only stock quantity, activation/deactivation, and unsaved-row discard for server-validated choice rows. The Products page and choice editor link to the complete Size/Color vocabulary manager, where canonical labels, grouped aliases, and active state can be reviewed and modified. Known Product Type, Tag, material, size, and color candidates appear automatically beside the description as suggestions only; explicit actions can transfer Size/Color candidates into unsaved choice rows and material candidates into unsaved material rows for review. Material correction/removal remains part of the atomic Product save, and material aliases are not learned automatically. Inventory controls, availability UI consumers, and full workspace screens are still deferred; computed availability, immutable adjustment-record integrity, the atomic `+1`/`-1` mutation service, and the ProductBundle stock boundary currently exist as backend foundations.
