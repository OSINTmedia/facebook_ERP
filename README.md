...
- An authenticated, Business-scoped Product Workspace route exists with deterministic compact cards and is verified locally.
- Business-scoped Product create/edit routes exist and are verified locally.
- Visible seller app workflows include login/logout, the foundation shell, the Product Workspace with compact lifecycle/availability cards and native exact-choice stock controls, Product create/edit with server-validated size/color choice rows and one-save Starting stock for new choices, read-only saved stock quantity, explicit activation/deactivation, and unsaved-row discard behavior, explicit confirmed Product Type/Tag controls, compact confirmed-material rows, a full Business-scoped Product Type/Tag/Size/Color vocabulary manager with grouped aliases and active state, and an automatic suggestions-only recognition preview beside the description. Size/Color candidates can be explicitly transferred into unsaved choice rows, and material candidates can be explicitly transferred into unsaved material rows; no candidate becomes confirmed truth until a valid Product bundle saves.
- Backend inventory foundations compute Product availability, expose an atomic Business/choice/actor-scoped `+1`/`-1` mutation service, record each accepted transition as an immutable adjustment fact, and keep ProductBundle editing from overwriting stock. Product edit retains compact HTMX server-truth controls for saved choices, while Workspace controls use native POST/redirect behavior so quantity, totals, and Availability are recomputed together. Workspace search validates canonical URL-backed query state against approved owned Product and confirmed catalog facts. Bounded filters and coherent Workspace HTMX replacement remain later Phase 6 work.
...
Seller-visible feature work currently includes email/password authentication, POST logout, the foundation shell, the Business-scoped Product Workspace with compact operational cards and native exact-choice stock controls, Product create/edit with atomic controlled-vocabulary choice-row validation and one-save Starting stock for new choices, a read-only ProductBundle stock boundary, and compact HTMX `+1`/`-1` controls for persisted choices on Product edit; contextual Size/Color creation; a complete Size/Color vocabulary manager with grouped aliases and active-state controls; automatic transient recognition feedback; explicit Size/Color candidate transfer into unsaved choice rows; and explicit material candidate transfer into editable unsaved material facts. The current verified setup covers dependency installation, the clean Django scaffold, environment-aware settings, PostgreSQL-only local runtime, migrations, authentication checks, auth-gated shell smoke checks, Business ownership tests, Product/choice/material form and bundle tests, Workspace scoping and card truth, create/edit integration, recognition/transfer no-write tests, cross-Business mutation rejection, vocabulary-update atomicity, forward/reverse migration-shape coverage for existing choices, the Business-scoped no-write availability service, adjustment-ledger integrity tests, atomic ProductBundle stock-boundary regression, and stock-route/HTMX/native-control integrated transition coverage.
...
After local PostgreSQL credentials are configured in `.env`, run `source .venv/bin/activate && python manage.py runserver 127.0.0.1:8000` to review the auth-gated shell locally. Anonymous users are redirected to the login page; after signing in, the Products nav opens the Product Workspace, whose compact cards separate lifecycle from computed Availability, show exact active-choice quantities, provide native `-1`/`+1` controls that return to a fully recomputed Workspace, and support validated URL-backed search across approved owned Product and confirmed catalog facts. Product create/edit supports controlled Business-scoped Size/Color dropdowns, contextual approved values and aliases, a read-only stock quantity field, compact HTMX `+1`/`-1` controls for each persisted choice, activation/deactivation, and unsaved-row discard for server-validated choice rows. The Products page and choice editor link to the complete Size/Color vocabulary manager, where canonical labels, grouped aliases, and active state can be reviewed and modified. Known Product Type, Tag, material, size, and color candidates appear automatically beside the description as suggestions only; explicit actions can transfer Size/Color candidates into unsaved choice rows and material candidates into unsaved material rows for review. Material correction/removal remains part of the atomic Product save, and material aliases are not learned automatically. Bounded filters and coherent Workspace HTMX replacement remain later Phase 6 work; computed availability, immutable adjustment-record integrity, the atomic `+1`/`-1` mutation service, and the ProductBundle stock boundary remain the governing backend foundations.
