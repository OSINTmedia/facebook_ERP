...
- Visible seller app workflows include login/logout, the foundation shell, the Product Workspace with compact lifecycle/availability cards, bounded search/filter state, and exact-choice HTMX stock controls with native fallback, Product create/edit with server-validated size/color choice rows and one-save Starting stock for new choices, read-only saved stock quantity, explicit activation/deactivation, and unsaved-row discard behavior, explicit confirmed Product Type/Tag controls, compact confirmed-material rows, a full Business-scoped Product Type/Tag/Size/Color vocabulary manager with grouped aliases and active state, and an automatic suggestions-only recognition preview beside the description. Size/Color candidates can be explicitly transferred into unsaved choice rows, and material candidates can be explicitly transferred into unsaved material rows; no candidate becomes confirmed truth until a valid Product bundle saves.
- Backend inventory foundations compute Product availability, expose an atomic Business/choice/actor-scoped `+1`/`-1` mutation service, record each accepted transition as an immutable adjustment fact, and keep ProductBundle editing from overwriting stock. Product edit retains compact choice-only HTMX controls. Workspace controls progressively enhance the same native POST route by replacing the complete server-rendered results region so quantity, totals, Availability, result count, and Availability-filter membership remain coherent. Workspace search and the bounded Lifecycle/Availability filters use validated canonical URL state over approved owned Product and confirmed catalog facts.
