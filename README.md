...
Seller-visible feature work currently includes email/password authentication, POST logout, the foundation shell, the read-only Product list, Business-scoped Product create/edit with atomic controlled-vocabulary choice-row validation, contextual Size/Color creation, a complete Size/Color vocabulary manager with grouped aliases and active-state controls, automatic transient recognition feedback, explicit Size/Color candidate transfer into unsaved choice rows, and explicit material candidate transfer into editable unsaved material facts. The current verified setup covers dependency installation, the clean Django scaffold, environment-aware settings, PostgreSQL-only local runtime, migrations, authentication checks, auth-gated shell smoke checks, Business ownership tests, Product/choice/material form and bundle tests, Product list scoping, create/edit integration, recognition/transfer no-write tests, cross-Business mutation rejection, vocabulary-update atomicity, forward/reverse migration-shape coverage for existing choices, the Business-scoped no-write availability service, and adjustment-ledger integrity tests.
...
After local PostgreSQL credentials are configured in `.env`, run `source .venv/bin/activate && python manage.py runserver 127.0.0.1:8000` to review the auth-gated shell locally. Anonymous users are redirected to the login page; after signing in, the root route renders the base application shell, the Products nav opens the Product list, and Product create/edit supports controlled Business-scoped Size/Color dropdowns, contextual approved values and aliases, quantity, active state, and removal for server-validated choice rows. The Products page and choice editor link to the complete Size/Color vocabulary manager, where canonical labels, grouped aliases, and active state can be reviewed and modified. Known Product Type, Tag, material, size, and color candidates appear automatically beside the description as suggestions only; explicit actions can transfer Size/Color candidates into unsaved choice rows and material candidates into unsaved material rows for review. Material correction/removal remains part of the atomic Product save, and material aliases are not learned automatically. Inventory controls, availability UI consumers, and full workspace screens are still deferred; computed availability, immutable adjustment-record integrity, and the atomic `+1`/`-1` mutation service currently exist as backend foundations.
