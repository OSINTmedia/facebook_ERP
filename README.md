...
Seller-visible feature work currently includes email/password authentication, POST logout, the foundation shell, the read-only Product list, Business-scoped Product create/edit with atomic controlled-vocabulary choice-row validation, contextual Size/Color creation, a complete Size/Color vocabulary manager with grouped aliases and active-state controls, automatic transient recognition feedback, and explicit Size/Color candidate transfer into unsaved choice rows. The current verified setup covers dependency installation, the clean Django scaffold, environment-aware settings, PostgreSQL-only local runtime, migrations, authentication checks, auth-gated shell smoke checks, Business ownership tests, Product/choice vocabulary model and form tests, Product list scoping, create/edit integration, recognition/transfer no-write tests, vocabulary-update atomicity, and forward/reverse migration-shape coverage for existing choices.
